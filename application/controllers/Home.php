...
class Home extends CI_Controller {
	...
	public function contact_us_form_submit()
	{
		...
		$EmailTo = "ashaduzzaman89@gmail.com";
		$Subject = "New Message Received from getconsumerproducts.com";
...
$from = $email;
$to = "ashaduzzaman89@gmail.com";
$subject = "Hi!";
$body = "Hi,\n\nHow are you?";
$host = "smtp.gmail.com";
$username = "mailsend@getconsumerproducts.com";
$password = "%afhE&%z3N.x";
 
require_once(APPPATH."third_party/phpmailer/PHPMailerAutoload.php");
 $mail 	= new PHPMailer();
        $mail->addReplyTo($to);   // Added by Akther Hussain date: 30-01-2017

        //$mail->From = $from;
        //$mail->FromName = $restaurant_name;
		...
		$mail->Port = 465;
		$mail->SMTPAuth = true;
		$mail->Username = "ashad.chefonline@gmail.com";
		$mail->Password = "pass123!@#chefonline";
		$mail->isSMTP();
		$mail->SMTPSecure = 'ssl';
		
        $mail->isHTML(true);
        $mail->Subject = $subject;
        //$msg = $this->load->view('email_templates/reservation_email_template.php',$data,true);

        $mail->Body = $msg;

        //return print_r($mail);exit;
	...
	public function sitemap_slug_create_order_menu()
	{
		...
		$get_categories =$this->Common_model->getData('*','adb_nav_menu', [ 'status' => 1,'cat_parent_id'=>0]);
		...
		foreach( $get_categories as $get_category ):
			...
			$catxml="\n<url><loc>".base_url('category/'.$get_category->slug)."</loc><changefreq>daily</changefreq><priority>0.84</priority></url>"; 
	
			$get_product_lists = $this->Product_model->getProductLimitDataNew(  '*', 'adb_product_list',0, 0,  'id', 'desc',  ['status'=>1,'parent_cat_id'=>$get_category->id] );
			//echo 'ashad=>'.count($get_product_lists);
			if(count($get_product_lists)==0)
			continue;
			//print_r($get_product_lists);exit;
				...
				$data='';
			if(isset($get_product_lists))
				foreach( $get_product_lists as $get_product_list ):

					$product_slug  = base_url('product/'.$get_product_list->slug);// order online

					$myfile     = fopen($myFile, "a") or die("Unable to open file!");
					
					$data       = $data."\n<url><loc>".$product_slug."</loc><changefreq>daily</changefreq><priority>0.64</priority></url>"; 
